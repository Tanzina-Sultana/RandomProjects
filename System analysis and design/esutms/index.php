...
<title>ESU Training calendar</title>
...
<table border="1" cellpadding="5" width="800">
	<tr>
		...
		<form action="book.php" method="post">
			...
			| <input name="item" type="radio" value="Swimming pool" />Class room 
			| <input name="item" type="radio" value="Drill" />Outdoor training | 
			...
			<p><input name="book" type="submit" value="Book" /></p>
		</form>
		</td>
		<td valign="top">
    		...
    		if (mysqli_num_rows($result) > 0) {
    			...
    			while($row = mysqli_fetch_assoc($result)) {
    				...
    				if($current_epoch == $row["start_day"] AND $current_epoch != $row["end_day"]) {
    					$calendar .= "Booking starts: " . sprintf("%02d:%02d", $row["start_time"]/60/60, ($row["start_time"]%(60*60)/60)) .sprintf($row["name"]) . "<br><hr><br>";
    				}
    				if($current_epoch == $row["start_day"] AND $current_epoch == $row["end_day"]) {
